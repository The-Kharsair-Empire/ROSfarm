#include "custom_rosplan_components/custom_METRICFFPlannerInterface.h"
...
namespace kharsair {
	...
	CustomMETRICFFPlannerInterface::CustomMETRICFFPlannerInterface(ros::NodeHandle& nh)
	{
		node_handle = &nh;
	...
	CustomMETRICFFPlannerInterface::~CustomMETRICFFPlannerInterface()
	{
		delete plan_server;
	}
	 ...
	 */
	std::string CustomMETRICFFPlannerInterface::runCommand(std::string cmd) {
		std::string data;
		FILE *stream;
		char buffer[1000];
	 ...
	 */
	bool CustomMETRICFFPlannerInterface::runPlanner() {
		...
		if (solved) {
			...
			while (line.find("Total cost") == line.npos and line.find("time spend") == line.npos and line.size() > 0) {
                if (line.substr(0, 9) == "plan cost") {std::cout << "\n\nSKIP PLAN COST 1\n\n"; break;}
                
				std::stringstream ss(line); // To trim whitespaces
				std::string aux;
				// Read the action number X:
				ss >> aux; //0: or n:
                if (aux.compare("plan cost:") == 0) { std::cout << "\n\nSKIP PLAN COST 2\n\n"; break;}
			...
			// std::cout << planner_output << std::endl;
	...
	int main(int argc, char **argv) {
		...
		kharsair::CustomMETRICFFPlannerInterface pi(nh);
